...
"""An implementation of PPO with IEM with TSallis entropy regularization with sparsemax support, with beta anneal.
...
class CategoricalMasked(Categorical):
    """A masked categorical supporting softmax or sparsemax heads."""
    ...
    def __init__(
        ...
        if use_sparsemax:
            ...
            logits = torch.where(masks, logits, mask_value)
...
class PPOAgent(nn.Module):
    ...
    def __init__(self, num_actions, observation_shape, device, policy_head="softmax"):
        super().__init__()
...
class PPO(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.policy_head = policy_head
        print(f"Tsallis q: {self.tsallis_q}, beta: {self.beta}, policy_head: {self.policy_head} with ANNEAL")
        ...
        self._adv_sum = 0.0
        self._adv_cnt = 0
        self._iem_r0_beta0_sum = 0.0
        self._iem_r0_beta0_cnt = 0
    ...
    def _accumulate_policy_probe_stats(
        ...
        support_fraction = support_nonzero.to(p.dtype) / torch.clamp(num_legal.to(p.dtype), min=1.0)

        # Top-k among legal actions only: push illegal actions to -1 before sorting probabilities
        ...
        # Descriptive policy spread metrics
        ...
        for player_id in (0, 1):
            ...
            s["sum_num_legal_actions"] += float(num_legal[mask].to(torch.float32).sum().item())
            s["sum_support_size_nonzero"] += float(support_nonzero[mask].to(torch.float32).sum().item())
            s["sum_support_fraction"] += float(support_fraction[mask].sum().item())
            s["sum_num_zero_prob_actions"] += float(num_zero[mask].to(torch.float32).sum().item())
            s["sum_num_small_positive_actions"] += float(num_small_pos[mask].to(torch.float32).sum().item())
    ...
    def step(self, time_step, is_evaluation=False):
        if is_evaluation:
            ...
            with torch.no_grad():
                ...
                self._accumulate_policy_probe_stats(probs, legal_actions_mask, current_players)
        ...
        if (self.iem_p1 is not None) and (self.iem_p0 is not None):
            ...
            with torch.no_grad():
                ...
                if mask0.any():
                    ...
                    self._iem_r0_cnt += 1
                    self._iem_r0_beta0_sum += float(b_rint[mask0].mean().item())
                    self._iem_r0_beta0_cnt += 1
        ...
        if self.total_steps_done - self.last_log_step >= self.log_interval:
            ...
            if self.track_visitation:
                ...
                log_data.update({
                    ...
                    "iem_rint_mean_p0_beta0": _safe_mean(self._iem_r0_beta0_sum, self._iem_r0_beta0_cnt),
                ...
                self._iem_r0_beta0_sum = 0.0; self._iem_r0_beta0_cnt = 0
