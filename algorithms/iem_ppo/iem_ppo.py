...
class PPO(nn.Module):
    ...
    def learn(self, time_step):
        ...
        if self.iem is not None:
            # Flatten obs buffer [T, N, D] -> [B, D]
            b_obs_full = self.obs.reshape((-1,) + self.input_shape)  # [B, D]


            _iem_loss = self.iem.update(b_obs_full.detach())

            with torch.no_grad():
                r_int = self.iem.intrinsic_reward(b_obs_full).reshape(self.steps_per_batch, self.num_envs)

            # mask out terminal steps (optional but sensible)
            r_int = r_int * (1.0 - self.dones)

            # add to extrinsic rewards BEFORE GAE
            self.rewards = self.rewards + r_int
