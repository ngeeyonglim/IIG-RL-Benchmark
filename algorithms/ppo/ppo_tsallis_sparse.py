...
"""An implementation of PPO with Tsallis Entropy with policy probing tracking with sparsemax.
...
def sparsemax(logits: torch.Tensor, dim: int = -1) -> torch.Tensor:
    """Sparsemax transform (Martins & Astudillo, 2016)."""
    ...
    k_z = support.to(torch.int64).sum(dim=dim, keepdim=True).clamp(min=1)
    tau = (z_cumsum.gather(dim, k_z - 1) - 1) / k_z.to(logits.dtype)
...
class PPO(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print("Using Tsallis-PPO all logging")
        ...
        self.policy_head = policy_head
        if self.policy_head not in {"softmax", "sparsemax"}:
            raise ValueError("policy_head must be either 'softmax' or 'sparsemax'.")
        print(self.policy_head, "policy head selected.")
